...
function main() {
	...
	while (count < arrHierarchy.length && counter < limit) {
		ret += getHTML(arrHierarchy[count]['name'], arrHierarchy[count]['value'], arrHierarchy[count]['color'], arrHierarchy[count]['index']);	
	 	count++;
	}
	
	/*while (count  && counter < limit && ) {
		ret += getHTML(arr[count]['name'], arr[count]['value'], arr[count]['color']);
		count++;	
	}*/
...
function getHTML(name, value, color, index) {
	rbv_api.println(name + "," + value + "," + color + "," + index);
	if (top8.indexOf(index) >= 0) {
		counter++;
